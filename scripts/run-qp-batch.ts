...
const testSamples: TestSample[] = [
  ...
  },
  {
    ...
    locale: "zh",
    user_answers: [1, 1],
    scene_tags: ["b1m_email_6_8_confirm", "b1m_standup_60_90s", "b1m_compare_options_reason", "b1m_handle_complaint_simple"],
    objective_score: 2,
    self_assessed_level: "B1",
    track_hint: "work"
  },
  ...
  {
    id: "sample_014",
    description: "冲突场景：自评B1但客观题0分",
    locale: "zh",
    user_answers: [0, 0],
    scene_tags: ["b1m_standup_60_90s", "b1m_email_6_8_confirm"],
    objective_score: 0,
    self_assessed_level: "B1",
    track_hint: "work"
  },
  ...
  {
    id: "sample_016",
    description: "工作场景密集测试",
    locale: "zh",
    user_answers: [1, 1],
    scene_tags: ["b1m_standup_60_90s", "b1m_email_6_8_confirm", "b1m_handle_complaint_simple", "b1m_compare_options_reason"],
    objective_score: 2,
    self_assessed_level: "B1",
    track_hint: "work"
  },
  {
    id: "sample_017",
    description: "日常生活场景密集测试",
    locale: "zh",
    user_answers: [0, 1],
    scene_tags: ["a1_basic_greeting_info", "a2_handle_counter_issue", "a2_read_service_notice"],
    objective_score: 1,
    self_assessed_level: "A2",
    track_hint: "daily"
  },
  {
    id: "sample_018",
    description: "英语+工作场景组合",
    locale: "en",
    user_answers: [1, 1],
    scene_tags: ["b1m_standup_60_90s", "b1m_email_6_8_confirm", "a2_polite_rephrase"],
    objective_score: 2,
    self_assessed_level: "B1",
    track_hint: "work"
  },
 ...
 * 发送单个测试请求
 */
async function sendTestRequest(sample: TestSample): Promise<BatchResult> {
 ...
 * 批量处理测试样本
 */
async function runBatchTest(samples: TestSample[]): Promise<BatchResult[]> {
 ...
 * 生成汇总报告
 */
function generateSummaryReport(results: BatchResult[]): BatchReport {
  ...
  const v1_v1_1_comparisons = successfulResults
    .filter(r => r.response.data.shadow_mode_enabled)
    .map(r => ({
      id: r.id,
 ...
 * 保存报告文件
 */
async function saveReports(report: BatchReport): Promise<void> {
...
async function main() {
  ...
  try {
    ...
    const healthCheck = await fetch(`${API_ENDPOINT.replace('/evaluate', '')}`, {
      method: 'GET',
      signal: AbortSignal.timeout(5000)
    });

    if (!healthCheck.ok) {
      throw new Error(`API端点不可用: ${healthCheck.status}`);
    }
...
export { runBatchTest, generateSummaryReport, saveReports };